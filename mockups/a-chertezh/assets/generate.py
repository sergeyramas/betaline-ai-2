...
# -*- coding: utf-8 -*-
"""
Betaline AI — «Чертёж» (Blueprint Calm).
Генерация диаграмм воображаемой науки об автоматизации:
изолинии, сетки узлов, траектории данных, тонкая гравюра.
Чернила + кобальт на тёплом белом. PIL + numpy, без внешних зависимостей.
"""
import math
import numpy as np
from PIL import Image, ImageDraw, ImageFont

BG      = (250, 250, 247)   # тёплый белый
INK     = (22, 24, 29)      # графитовые чернила
GRAY    = (150, 152, 156)   # служебный серый
FAINT   = (222, 221, 214)   # линии сетки
FAINT2  = (206, 205, 197)
COBALT  = (36, 71, 240)     # единственный акцент
SS      = 2                 # суперсэмплинг
...
def hero():
    ...
    draw_isolines(dr, F, levels, (x0, y0, x1, y1), (196, 196, 190))
    # две ключевые изолинии — чернилами и кобальтом
    draw_isolines(dr, F, [levels[7]], (x0, y0, x1, y1), (120, 122, 126))
    ...
    for _ in range(26):
        ...
        for _s in range(90):
            ...
            pts.append((px, py))
        if len(pts) > 12:
            dotted(dr, pts, (172, 173, 170), r=0.9, step=8.5)
    ...
    for (a, b) in sorted(edges):
        ...
        if (a, b) in cobalt_edges:
            ...
            polyline(dr, pts, (108, 110, 115), width=1)
    ...
    save(img, "/home/user/betaline-ai-2/mockups/a-chertezh/assets/hero-diagram.png", W, H)
...
def process_band():
    ...
    polyline(dr, upper, (150, 152, 156), width=1)
    polyline(dr, lower, (196, 196, 190), width=1)
    ...
    save(img, "/home/user/betaline-ai-2/mockups/a-chertezh/assets/process-band.png", W, H)
...
def field_study():
    ...
    draw_isolines(dr, F, levels, (x0, y0, x1, y1), (200, 200, 194))
    draw_isolines(dr, F, [levels[10]], (x0, y0, x1, y1), (118, 120, 124))
    ...
    for _ in range(3400):
        ...
        if rng.random() < v ** 2.2:
            ...
            y = y0 + py * (y1 - y0)
            r = 0.9 + 0.9 * v
            dr.ellipse([L(x - r), L(y - r), L(x + r), L(y + r)], fill=(90, 92, 97))
    ...
    save(img, "/home/user/betaline-ai-2/mockups/a-chertezh/assets/field-study.png", W, H)
