...
- [2026-09-22 08:55 UTC] **claude-mac-opus5** (Mac) — topic: `all-sites-to-base-vps` — DONE. После custom2 (см. ниже) по команде оператора перенесены и апекс betaline-ai.ru + www, custom, zvonok: свои `<name>-api` юниты (:8792/:8791) с env из соответствующих Vercel-проектов, nginx same-origin `/api`, LE-сертификаты, A-записи через Beget API (апекс — с сохранением MX/SPF). check-host ru1–ru3 = 200 на всех, тестовые лиды апекс/zvonok `ok:true`. `tools/deploy-vps.sh custom2|custom|api|apex|zvonok`. Vercel — резерв.
- [2026-09-22 08:40 UTC] **claude-mac-opus5** (Mac) — topic: `custom2-to-base-vps` — DONE. custom2.betaline-ai.ru и новый api.betaline-ai.ru переведены с Vercel на base-vps (217.60.249.216): Vercel-IP частично блокируют в РФ (Андрей+Дима, 21.09). Сделано: `server.js` (shim над `api/*.js`, systemd `betaline-api`), `/srv/betaline/{api,custom2}`, nginx + LE до 21.12, A-записи через Beget API, cron close-stale, `tools/deploy-vps.sh`, `main.js` API_BASE → api.betaline-ai.ru. Проверено: check-host ru1/ru2/ru3 = 200 (ru1 на Vercel был timeout), тестовые лиды `ok:true`, чат отвечает. 🔴 Открыто: ключи Google Sheets и Resend мертвы (и на Vercel тоже); `custom` и апекс пока на Vercel.
