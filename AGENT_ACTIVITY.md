...
- [2026-09-24 UTC] **Betaline-правки (Sonnet)** — чат-виджет шлёт в `/api/chat-ai` полный адрес страницы
  (с доменом), чтобы бот различал custom/custom2/апекс; PR в master, ждёт деплоя оператором. SHA: `757feb3`.

- [2026-09-22 UTC] **Betaline-правки (Sonnet)** (VPS, задание диспетчера, СРОЧНО по прямому «Делай» владельца) —
  topic: `ai-chat-ban-close` — код готов, **master `7b2315d`** (запушено напрямую), **НЕ ВЛИЯЕТ НА ЖИВОЙ САЙТ**.
  🔴 Главная находка: живой AI-чат-бот работает не из этого репозитория (`api/` здесь — мёртвая копия, CLAUDE.md
  §4), боевой код — в `sergeyramas/betaline-landing`, доступа туда из VPS-воркtree нет. Спам троллей продолжается,
  пока кто-то с доступом к `betaline-landing` не перенесёт патч. Владельцу предложен единственный по-настоящему
  немедленный стоп-кран без деплоя — revoke ключа OpenAI в его дашборде (бьёт по всем посетителям, не только
  трою).
  Реализовано: бан/разбан/закрытие AI-диалога — до вызова OpenAI проверяем статус визитора/темы (fail-open при
  сбое Sheets), инлайн-кнопки «Забанить»/«Закрыть чат» на вступительном сообщении темы (кнопка отражает текущий
  статус), новый `api/telegram-webhook.js` — бот раньше только отправлял сообщения, теперь и принимает
  `callback_query` (admin allowlist `TELEGRAM_ADMIN_IDS`, `secret_token`, `answerCallbackQuery` всегда, паттерн
  из `mpoto/pcmarket-ai-seller`). Лист «Баны» + новые колонки `visitorId`/`infoMessageId` в «Топики»
  (`api/_lib/sheets.js`).
  Код-ревью (сабагент) нашло и я тут же дофиксил коммитом `7b2315d`: (1) admin-check в вебхуке был fail-open
  при не настроенном `TELEGRAM_ADMIN_IDS` — теперь fail-closed; (2) фронт не знал про `data.muted` — банило
  сервер, но пузырь пустого ответа бота рисовался и цель `chat_message` стреляла на спам-сообщения, засоряя
  конверсионную статистику — починено в `main.js`.
  Ключ бана — `visitorId` (localStorage), не IP: честное ограничение, обходится очисткой/инкогнито.
  Требует нового: `TELEGRAM_ADMIN_IDS`, `TELEGRAM_WEBHOOK_SECRET` в env боевого проекта, ручная регистрация
  вебхука (`setWebhook`), вкладка «Баны» в Google-таблице (Sheets API сам вкладки не создаёт).
  Ревью Codex запущено постфактум (не блокировало выкатку, как и просил владелец) — вердикт придёт письмом.
  Открытые вопросы к владельцу (не решал сам): срок бана навсегда/N дней, поведение при повторном заходе,
  пересылать ли сообщения забаненного в Telegram с пометкой или не пересылать вовсе.
