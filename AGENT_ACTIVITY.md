...
- [2026-09-07 07:40 UTC] **claude-mac-fable51** (Mac) — topic: `blueprint-site` — ветка `custom-subdomain`
  Сборка сайта из утверждённого макета `mockups/blueprint-v2/` (решение оператора 07.09: макет как есть,
  включая цены 150k–1M). Переносим макет в корень (index.html/style.css/main.js), обвязка с v3: форма →
  боевой `/api/lead` (source=audit), чат-виджет + callback, модалки политики/оферты, реквизиты, SEO-голова.
  Партиалы упраздняются (сайт ~30 КБ, один файл). Дальше: агент-сверщик с макетом + Codex-ревью.
