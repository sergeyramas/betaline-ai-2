...
| Production URL | будущий: `https://betaline-ai.ru` (сейчас там старый сайт из репо betaline-landing). 🔴 **С 22.09 ВСЕ сайты Betaline (апекс, www, custom, custom2, zvonok) и API — на base-vps 217.60.249.216**, Vercel-проекты остались как резерв (откат = A-записи на 76.76.21.21). Схема: nginx `/etc/nginx/sites-available/betaline`, статика `/srv/betaline/{apex,custom,custom2,zvonok}`, API — три systemd-юнита `betaline-api` :8790 (api.betaline-ai.ru, для custom/custom2), `betaline-apex-api` :8792 (same-origin `/api` апекса), `betaline-zvonok-api` :8791; сертификаты LE до 21.12.2026. `www` раньше вёл на старый Beget-хостинг (5.101.152.161, древний сайт без HTTPS) — теперь на VPS |
...
| Поддомен-2 | **`https://custom2.betaline-ai.ru` — с 22.09 живёт на base-vps (217.60.249.216), не на Vercel** — Vercel-IP `76.76.21.21` частично блокируют в РФ (у Андрея и Димы, Ростелеком + мобильный, сайт не открывался 21.09; check-host: узел ru1 timeout). nginx `/etc/nginx/sites-available/betaline`, статика `/srv/betaline/custom2`, сертификат LE до 21.12.2026. Vercel-проект `betaline-ai-2-custom2` остался как резерв (откат = вернуть A на 76.76.21.21). Тот же сайт, но hero-картинка — исходная топокарта макета вместо инлайн-SVG-схемы (единственное отличие). Отдельный Vercel-проект `betaline-ai-2-custom2` (`prj_6MnG6pIBY1SLHPTBKTIPpaOmnss2`), та же команда npz-avod. Свой счётчик Метрики **`112650916`** «Betaline AI — custom2.betaline-ai.ru» (заведён 15.09 в том же `gowindo-elama1`, те же 4 цели; генератор подменяет YM_ID сам). Генерируется скриптом `tools/build-custom2.py` из корневого `index.html` — не отдельная ветка, см. §5. **Тарифы на custom2 другие** (решение оператора 19.09): от 150 / 450 / 600 / 750 / 900 тыс. ₽ — подменяет генератор (`PRICES`), на custom макетные |
...
| Боевой API | `https://betaline-ai.ru/api/lead`, `/api/chat-ai` (CORS `*`) — на апексе (Vercel). **Поддомены с 22.09 ходят в `https://api.betaline-ai.ru`** — те же `api/*.js` на base-vps через `server.js` (systemd `betaline-api`, порт 8790, каталог `/srv/betaline/api`, env скопирован с Vercel-проекта апекса в `/srv/betaline/api/.env`), cron `close-stale` 09:00 в crontab root. 🔴 Sheets (`Invalid JWT Signature`) и Resend (`API key is invalid`) падают и там, и на Vercel — ключи мертвы, лид всё равно `ok:true` (Telegram + CRM работают) |
...
| деплой custom | **`tools/deploy-vps.sh custom`** (корень репо → base-vps). `vercel deploy --prod` больше не деплоит сайт — DNS смотрит на VPS |
| перелить custom → custom2 | **`tools/deploy-vps.sh custom2`** (build-custom2.py + rsync на base-vps) — после каждого прод-деплоя custom предложить оператору перелить в custom2. Старый путь через Vercel (`cd dist-custom2 && vercel deploy --prod --yes --scope npz-avod`) — только для резервного проекта; 🔴 `dist-custom2/.vercel/project.json` обязан существовать, без него deploy заводит дубль-проект `dist-custom2` |
| деплой апекса / zvonok | `tools/deploy-vps.sh apex` (из worktree betaline-master, статика + `apex-api` :8792) / `tools/deploy-vps.sh zvonok` (betaline-voice-ai, статика + `zvonok-api` :8791). У каждого свой `api/` и `.env` в `/srv/betaline/<name>-api/` |
| деплой api на VPS | `tools/deploy-vps.sh api` — после правки `api/`, `bot/betaline_kb.txt` или `server.js`. Логи: `ssh base-vps journalctl -u betaline-api -f` |
...
- **`API_BASE` в main.js — точное совпадение хоста, не `endsWith`.** `custom.betaline-ai.ru` тоже оканчивается на `betaline-ai.ru`, а своих env у проекта нет — с `endsWith` формы постили бы в пустой локальный `/api/lead` и лиды терялись молча. Same-origin только на апексе; все остальные хосты (custom, custom2, превью) → `https://api.betaline-ai.ru` (base-vps, с 22.09).
- **Beget DNS API: `dns/getData` на поддомене отдаёт `METHOD_FAILED` даже когда запись есть** (custom2 22.09) — не признак отсутствия; проверять `dig @ns1.beget.com`, `changeRecords` работает. Креды — `~/.secrets/api_keys.env` (`BEGET_API_LOGIN`/`BEGET_API_PASSWORD`).
