...
- **«Прогноз конверсии»** — вырастет / упадёт / нейтрально, уверенность
  (высокая/средняя/низкая) и за счёт чего.

**Запрет на выдуманные цифры.** «+15% к заявкам» без данных — не прогноз, а число с
потолка. Вилка с пометкой «оценка» допустима, только когда есть на что опереться;
иначе — направление и механизм словами. Нет оснований судить — писать «не могу
оценить, нужно то-то».
