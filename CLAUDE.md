...
## 8a. Порядок работы с заказчиком (Андрей) — план до правки

**Правило (решение оператора, 17.09.2026):** получив задачу от Андрея (через RamOS-чат,
Telegram или GitHub Issue — каналы в `docs/agents/ANDREY.md`), агент СНАЧАЛА отвечает
планом и только потом правит сайт. Не наоборот.

Why: 15.09 в соседнем проекте заказчик прислал готовый чужой код с подписью «примени» —
без промежуточного плана это едва не заменило рабочую логику целиком. План до правки —
защита с обеих сторон: заказчик видит, что получит, до того как это попадёт на сайт под
рекламным трафиком; агент не бежит исполнять чужой патч вслепую.

План включает:
1. **Что именно будет изменено** — какие блоки/тексты/файлы.
2. **Как это будет выглядеть** — словами, для человека без кода: что увидит на экране,
   где, вместо чего.
3. **Рекомендации** — если есть вариант лучше/короче/безопаснее, предложить его и
   объяснить разницу в одну строку. Если просьба ломает что-то существующее (вёрстку,
   аналитику, рекламные метки) — сказать об этом ДО правки, не после.
4. **Явный вопрос «делаю?»** — и ждать подтверждения Андрея или Сергея. Молчание,
   «ок наверное», «делай как лучше» подтверждением НЕ считаются.

После подтверждения — правка по обычному контуру (`docs/agents/ANDREY.md`: ветка
`andrey/<slug>`, QA, PR) и отчёт по факту: что изменено, где смотреть.

**Исключение — без плана можно править сразу:** явная опечатка или битая ссылка, где
вариант правки ровно один (нет развилки, нечего утверждать). В этом случае — правка сразу,
с пометкой в ответе «сделал без плана, вот что». Любая правка, где есть выбор (формулировка,
цвет, расположение, что показывать вместо чего) — под общее правило, план обязателен.
