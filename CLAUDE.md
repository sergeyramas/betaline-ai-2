...
| создать поисковую кампанию по API | `python3 tools/direct-create-search-campaign.py --dry-run` → без флага создаёт и ставит на паузу. Фразы/тексты/минус-слова — константы в скрипте |
| мониторинг Директа | `python3 tools/direct-monitor.py --dry-run` (без `--dry-run` шлёт оператору). Копия для launchd — `~/.local/bin/betaline-direct-monitor.py`, после правки скрипта: `cp tools/direct-monitor.py ~/.local/bin/betaline-direct-monitor.py`. Лог: `~/Library/Logs/betaline-direct-monitor.log`. 🔴 launchd не выполняет файлы из `~/Documents` (TCC, exit 126) — поэтому и скрипт, и `tg-send` живут копиями в `~/.local/bin/` (16–20.09 отчёты 5 дней не доходили именно из-за этого) |
...
**Дополнение (решение оператора, 17.09.2026).** Не всякая правка идёт по маршруту
«план → «да» Андрея → код» напрямую. Есть второй, более тяжёлый класс.

- **ОБЫЧНАЯ правка** — текст, заголовок, картинка, цвет, шрифт, мелкая вёрстка: маршрут
  как в §8a выше (план Андрею, его «да», правка).
- **ГЛОБАЛЬНАЯ правка** — меняет логику или устройство сайта, а не только внешний вид.
  Маршрут другой, и агент НЕ идёт к Андрею сразу:
  1. агент оформляет план;
  2. план уходит на **независимое ревью Codex** — чат «Ревизор (Codex) — ревью
     глобальных правок» (id `eaad2dab-37e7-47f8-a9b2-17ef9cec8bc7`, тот же проект
     betaline-ai-2), обращение через agent-mail с заголовком `X-RamOS-Chat-Id` этого
     чата (дополнение оператора 17.09.2026);
  3. вердикт ревьюера прикладывается к плану;
  4. план оформляется как **Вопрос владельцу (Сергею)** через механизм «Вопросы» в
     RamOS, вердикт Codex идёт вместе с ним. Сергей либо подписывает план как есть,
     либо правит формулировку;
  5. только утверждённый Сергеем вариант (с вердиктом) уходит Андрею на согласование,
     и лишь после «да» Андрея делается правка. Если Андрей не согласен с утверждённым
     вариантом — возврат к Сергею; агент сам компромисс не выбирает.

  **Ревью Codex — правила.** Read-only: ревьюер разбирает риски, не правит код и
  ничего не выкатывает. Отвечает по пунктам: что ломается на живом сайте; задевает ли
  правка рекламные метки и цели Метрики (Директ ведёт на custom2 — это живой трафик);
  формы и куда уходят заявки; по-разному ли правка ложится на custom и custom2; чем
  откатывать; нужен ли новый доступ или внешний сервис. Если ревьюер нашёл риск уровня
  «ломает» — план владельцу не уходит, пока агент не переделает план или прямо не
  напишет, почему риск принимается. Если ревьюер недоступен (модель не отвечает, лимит,
  ошибка) — глобальная правка НЕ делается молча, Codex агент собой не подменяет: пишет
  владельцу «ревью недоступно, причина» и ждёт решения. На обычные правки это ревью не
  распространяется — там маршрут прежний (см. §8a выше).

  🔴 **Технически (проверено учениями 17.09.2026, задания d7753311/aadff068):**
  агент НЕ может прочитать ответ ревьюера опросом в том же задании — чтение чужого
  чата закрыто платформой (`RAMOS_AGENT_MAIL_CHAT_URL` на чат ревьюера отдаёт HTTP 403
  `{"error":"dispatch_not_enabled"}` для обычного, не диспетчерского задания; отправлять
  можно, читать чужой чат — нет, это привилегия диспетчера). Вердикт ревьюера доходит
  только письмом обратно в ЭТОТ чат — отдельным, новым заданием, не продолжением
  текущего разговора; **проверено на практике 17.09.2026**: вердикт по учебному плану
  пришёл межагентским письмом из чата ревьюера и продолжил прогон следующим заданием.
  Значит Шаг 2 и Шаг 3 всегда живут в РАЗНЫХ заданиях — это норма маршрута, а не сбой. Если в разумный срок такое письмо не пришло (или
  видно, что вердикт не оттуда) — не пытаться повторно вычитывать чужой чат, а писать
  `ревью не пришло: <точная причина>` и передавать дальше человеку/диспетчеру: он может
  прочитать чат ревьюера напрямую и переслать план вручную.

  **Кто решает что (уточнение оператора, 17.09.2026).** Признать ли правку глобальной —
  решает агент сам, по списку признаков ниже; если признал — ОБЯЗАН отправить план
  письмом в комнату ревьюера сам, не спрашивая разрешения и не пропуская шаг 2/5 ради
  скорости. Это обязанность, не выбор. А вот КТО ревьюер и на какой модели он фактически
  работает — настройка комнаты «Ревизор (Codex)», а не решение агента внутри задания:
  агент не выбирает себе проверяющего, иначе проверку назначает тот, кого проверяют.
  Отдельно от этого — галочки «Контролёры» в настройках чата: они проверяют работу
  внутри того же задания и на той же модели, это самопроверка. Полезная, но НЕ замена
  ревьюеру из отдельной комнаты и не повод пропускать шаг 2/5.

К глобальным относится (стартовый список, Сергей поправит по ходу):
...
**Разрешение спорных случаев:** не уверен, обычная правка или глобальная — считать
глобальной и идти к Сергею. Ошибка в эту сторону стоит одного лишнего вопроса, ошибка в
другую — сломанный лендинг под живой рекламой.

**Как оформлять «Вопрос» технически (проверено 17.09.2026):** у задания агента (job) нет
API-пути создать запись в `questions` от своего имени — оба маршрута создания
(`POST /api/projects/:id/questions`, `POST /api/chats/:id/questions`) в `api/questions.ts`
защищены `requireAuth` (сессия живого пользователя), это осознанное решение платформы:
«Вопрос» заводится нажатием кнопки человеком, а не агентом-парсером своих сообщений —
внутреннего/job-токен маршрута для этого нет. Пока платформа не даст агенту такой путь,
**временный протокол**: план глобальной правки агент оформляет обычным сообщением в этом
чате, и **первая строка сообщения — дословно `НА ПОДПИСАНИЕ СЕРГЕЮ`**. Сам вопрос в
разделе «Вопросы» RamOS в этом случае заводит человек (Сергей или тот, кто увидел
пометку), не агент.

**Видимость хода работы (решение оператора 17.09.2026).** Ход глобального сценария
обязан быть виден в чате по шагам, не только в финальном отчёте: владелец должен
понимать, где работа стоит прямо сейчас, не дожидаясь конца. Каждый шаг — отдельное
сообщение в момент, когда шаг случился, ничего не собирать в один итог задним числом:

1. `🌍 ГЛОБАЛЬНЫЙ СЦЕНАРИЙ. Шаг 1/5 — план.` Дальше: почему правка признана глобальной
   (какой именно пункт списка выше сработал — блоки/формы/аналитика/рекламные
   метки/цены/два поддомена/новый доступ), сам план, как будет выглядеть результат — и
   три обязательные строки «Цель / Аудитория / Прогноз конверсии» (правило ниже).
2. `Шаг 2/5 — план ушёл ревьюеру (Codex), жду вердикт.` Указать время отправки.
3. `Шаг 3/5 — вердикт ревьюера: безопасно / рискованно / ломает.` Короткая выжимка в
   первых строках, полный текст разбора ниже — включая согласие или несогласие
   ревьюера с прогнозом конверсии из плана (правило ниже). Агент ОБЯЗАН назвать модель,
   на которой фактически работал ревьюер; если она совпала с моделью самого агента —
   писать прямо «ревью не независимое, вердикт носит совещательный характер» и не
   выдавать его за внешнюю проверку.
4. `Шаг 4/5 — ТРЕБУЕТСЯ ВАША ПОДПИСЬ.` Первой строкой именно эта пометка, дальше план и
   вердикт вместе. После этого сообщения работа стоит и ничего не правится, пока
   владелец не ответил.
5. `Шаг 5/5 — подписано, ушло Андрею на согласование.` После согласования Андрея —
   правка, затем отдельное `Сделано` с тем, что изменилось, как откатить — и как
   проверить прогноз конверсии (правило ниже, попадает в тот же отчёт).

Для обычной правки (маршрут §8a выше) — одна строка в начале сообщения: `Обычная
правка` и причина, чтобы разница между двумя маршрутами читалась с первого сообщения.

Если шаг сорвался (ревьюер недоступен, вердикт «ломает», Андрей не согласен) — об этом
пишется отдельным сообщением сразу, а не молчание до конца. Пропущенный шаг — это
нарушение правила, а не оптимизация скорости.

### 8a.2 Цель, аудитория, прогноз конверсии

**Правило (решение оператора, 17.09.2026).** Любая правка оценивается против цели
сайта и аудитории, и по ней даётся прогноз конверсии.

В плане ГЛОБАЛЬНОЙ правки (Шаг 1/5 выше) обязательны три строки:
- **«Цель»** — что считается успехом на этой странице (заявка, звонок, переход в
  мессенджер) и как правка на это работает;
- **«Аудитория»** — кто эту страницу читает и откуда пришёл (у проекта два поддомена:
  custom2 — под живой рекламой Директа, custom — без неё, трафик разный);
- **«Прогноз конверсии»** — человеческими словами, одной-двумя фразами, отвечает на
  ДВА вопроса сразу: (1) заявок станет больше или меньше; (2) КТО начнёт приходить —
  тот же клиент, более бюджетный или более дорогой (дополнение оператора 17.09.2026).

**Тон прогноза (дополнение оператора 17.09.2026).** Не таблица, не проценты, не
наукообразие — обычная фраза, как сказал бы человек коллеге. Два образца, формат
именно такой:
- «Судя по нашей аудитории и цели страницы, конверсия в заявку скорее всего
  просядет — заголовок начнёт цеплять тех, кто ищет подешевле, а они до заявки
  доходят реже.»
- «Заявок может стать больше, но это будет более бюджетная аудитория: средний чек
  ниже.»

**Сдвиг аудитории — такой же результат, как рост или падение числа заявок.** Правка
может поднять количество обращений и при этом ухудшить дело, если приводит более
бюджетного клиента — по цифрам всё выглядит лучше, а по сути хуже. Если правка
смещает аудиторию — писать это прямым текстом, даже когда число заявок растёт.
Молчание про сдвиг аудитории — не «прогноз без деталей», а неполный прогноз.

**Запрет на выдуманные цифры.** «+15% к заявкам» без данных — не прогноз, а число с
потолка. Проценты и вилки не нужны вообще — см. тон выше; если очень хочется
количественную оценку, и есть на что опереться, — направление словами, не процент.
Нет оснований судить — писать «не могу оценить, нужно то-то».

**Проверяемость.** К каждому прогнозу — как его проверить после выката: какая цель
Яндекс.Метрики, за какой срок, с чем сравнивать. То же самое попадает в отчёт
`Сделано` (Шаг 5/5), чтобы через неделю можно было вернуться и посмотреть, сбылось
или нет.

**Ревьюер** (Codex, §8a.1) разбирает план по тем же трём блокам и может не
согласиться с прогнозом — тогда в вердикте два мнения, и владелец подписывает, видя
оба. Агент не подгоняет свой прогноз под ревьюера, и наоборот.

**На ОБЫЧНЫЕ правки** три блока не обязательны — кроме случая, когда правка трогает
заголовок, цену, кнопку или форму: там прогноз нужен даже у мелкой правки.

Оценка идёт против документа `docs/ads/AUDIENCE-GOAL.md` (цель сайта и портрет
аудитории) — если файла ещё нет (черновик на подписи у Сергея на момент этой записи,
17.09.2026), оценивать по тому, что реально известно про два поддомена и рекламу
(§2 CLAUDE.md, `docs/ads/MONITORING.md`), и явно писать, на что опирается прогноз.
