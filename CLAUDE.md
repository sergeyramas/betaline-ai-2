...
| Поддомен-2 | **`https://custom2.betaline-ai.ru` — работает с 15.09**, тот же сайт, но hero-картинка — исходная топокарта макета вместо инлайн-SVG-схемы (единственное отличие). Отдельный Vercel-проект `betaline-ai-2-custom2` (`prj_6MnG6pIBY1SLHPTBKTIPpaOmnss2`), та же команда npz-avod. Свой счётчик Метрики **`112650916`** «Betaline AI — custom2.betaline-ai.ru» (заведён 15.09 в том же `gowindo-elama1`, те же 4 цели; генератор подменяет YM_ID сам). Генерируется скриптом `tools/build-custom2.py` из корневого `index.html` — не отдельная ветка, см. §5. **Тарифы на custom2 другие** (решение оператора 19.09): от 150 / 450 / 600 / 750 / 900 тыс. ₽ — подменяет генератор (`PRICES`), на custom макетные |
